...
/* FEC Encoder function */

/**************************************************************************************************************
* @data_in  is pointer (malloced and filled) with the input data to the encoder
    data in contains a prn9 whitehened sequence plus crc (2B).
  @data_out already malloced pointer to data output, lenght of data out must be at least:
    (len + 2) * 2
  @len length of input data
*/

unsigned short fecEncode(uint8_t * data_in, uint8_t * data_out, int len){
    uint16_t fecEncodeTable[] = { 
        0, 3, 1, 2, 
        3, 0, 2, 1, 
        3, 0, 2, 1, 
        0, 3, 1, 2    
    };  
    uint16_t fec_len = (uint16_t) (len+2)*2;
    uint16_t i, j, val, fecReg, fecOutput; 
    uint32_t intOutput; 
    uint8_t original[len + 2];
    uint8_t fec[fec_len]; 
    uint8_t interleaved[fec_len]; 
    uint16_t checksum; 
    uint8_t length; 
    memcpy(original, data_in, len);
    // Append Trellis Terminator 
    original[len] = 0x0B; 
    original[len + 1] = 0x0B; 
    fecReg = 0;  
    for (i = 0; i < (len+2); i++) { 
        fecReg = (fecReg & 0x700) | (original[i] & 0xFF); 
        fecOutput = 0; 
    for (j = 0; j < 8; j++) { 
    ...
    memcpy(data_out, interleaved, fec_len);
    return (unsigned short) fec_len;
}
